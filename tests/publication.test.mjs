...
test('seven production kits contain finished, distinct deliverables',()=>{
 ...
 for(let d=1;d<=7;d++){
  ...
  assert.ok(k.finishedLongform.opening.length>120);assert.equal(k.finishedLongform.sections.length,4);assert.equal(k.finishedVertical.spokenScript.length,7);assert.equal(k.finishedCarousel.length,7);assert.ok(k.platformCopy.instagram.length>150);assert.ok(k.platformCopy.linkedin.length>250);assert.match(k.platformCopy.youtubeDescription,/Claims, rights treatments, transcript and sources/);assert.match(md,/## Canonical essay/);assert.match(md,/## Long-form video script/);assert.match(md,/## Instagram carousel/);assert.ok(md.length>8000,`day ${d} production markdown depth`);
